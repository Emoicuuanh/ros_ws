#include "my_robot_planning/dijkstra_planner.hpp"
...
{
    RCLCPP_INFO(node_->get_logger(), "CLeaning up plugin %s of type DijktraPlanner",name_.c_str());
}
void DijkstraPlanner::activate()
{
    RCLCPP_INFO(node_->get_logger(), "Activating up plugin %s of type DijktraPlanner",name_.c_str());
}
void DijkstraPlanner::deactivate()
{
    RCLCPP_INFO(node_->get_logger(), "Deactivating up plugin %s of type DijktraPlanner",name_.c_str());
}
...
PLUGINLIB_EXPORT_CLASS(my_robot_planning::DijkstraPlanner, nav2_core::GlobalPlanner)