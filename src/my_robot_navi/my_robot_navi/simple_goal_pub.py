...
class WaypointPublisher(Node):
    def __init__(self):
        ...
        self.state_pub = self.create_publisher(Bool,"/state",10)
        self.start_sub = self.create_subscription(
            Bool,
            "/start_navi",      # topic bạn muốn
            self.start_callback,
            10
        )
    ...
    def load_waypoints(self):
        ...
        try:
            with open(FILE_PATH, 'r') as f:
                ...
                if not content:
                    self.get_logger().warn("⚠️ File waypoint rỗng — không có dữ liệu!")
                    return []

                data = json.loads(content)

                # Nếu dữ liệu không phải list thì báo lỗi
                if not isinstance(data, list):
                    self.get_logger().error("❌ File waypoint không phải dạng LIST!")
                    return []

                return data

        except Exception as e:
            self.get_logger().error(f"❌ Lỗi đọc JSON: {e}")
            return []
    ...
    def start_callback(self, msg):
        if msg.data:
            self.get_logger().info("🟢 Nhận tín hiệu START — load lại waypoint!")

            # Load lại file JSON
            self.waypoints = self.load_waypoints()

            if not self.waypoints:
                self.get_logger().error("❌ Không có waypoint — không thể chạy!")
                return
